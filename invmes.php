...
<body>
 <?php 	
   ...
   // SELECT * FROM `inventario` WHERE MONTH(`fecha`) = 04 AND YEAR(`fecha`) = 2012;
	if (isset($_GET['mes']) ) { 
	$mes = $_GET['mes'];  
    $num_pagina2 = 0;
	if (isset($_GET['anio']) ) { 
...
echo "<table border=1 id='el-primer-parrafo'>"; 
	...
	echo "</tr>"; 
$result = mysql_query("SELECT * FROM `inventario` WHERE MONTH(`fecha`) = $mes AND YEAR(`fecha`) = $anio ") or trigger_error(mysql_error()); 
